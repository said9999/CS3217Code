...
- (void)viewDidLoad
{
   ...
   // [_wolf.view addGestureRecognizer:pan];
    ...
    _wind = [[GamePorjectile alloc] initWithX:0 Y:0];
    _wind.delegate = self;
    
    _difProjectile = [[GamePorjectile alloc] initWithX:0 Y:0];
...
- (IBAction)buttonPressed:(id)sender {
    ...
    if ([[button title] isEqual:RESET]) {
      ...
      //  self.isSavedInThisGame = YES;
        ...
        [self gameOver];
    }else if([[button title] isEqual:@"Level 1"]){
        printf("Level1");
        [self loadFileWithIndex:0 levelInfo:@"Level-1"];
    }else if([[button title]isEqual:@"Level 2"]){
        [self loadFileWithIndex:0 levelInfo:@"Level-2"];
    }else if([[button title]isEqual:@"Level 3"]){
        printf("Level 3");
        [self loadFileWithIndex:0 levelInfo:@"Level-3"];
    }
}
